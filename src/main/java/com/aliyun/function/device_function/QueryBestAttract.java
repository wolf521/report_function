 ...
 *
 */ 
public class QueryBestAttract implements PojoRequestHandler<Map<String, Object>, Map<String, Object>> {
	...
	public Map<String, Object> handleRequest(Map<String, Object> requestParam, Context context) {
		...
		context.getLogger().info("查询最佳吸引数据参数：" + requestParam);
		try {
			// 获取参数
			...
			response.success(new DeviceServiceImpl().queryBestAttract(queryParameters.get("unitId").toString()));
			result.put("body", response);
		} catch (Exception e) {
			context.getLogger().info("查询最佳吸引数据异常：" + e.getMessage());
		}
		return result;
	}
}
