 ...
 * @return
 */
public class QueryTrendChart implements PojoRequestHandler<Map<String, Object>, Map<String, Object>> {
	...
	public Map<String, Object> handleRequest(Map<String, Object> requestParam, Context context) {
		...
		try {
			...
			response.success(new DeviceServiceImpl().queryTrendChart(paramBody));
			context.getLogger().info("查询单展台折线图参数："+paramBody);
			result.put("body", response);
		} catch (Exception e) {
			context.getLogger().info("查询单展台折线图异常："+e.getMessage());
		}
		return result;
	}
}
